...
int main()
{
	...
	void* pFile = OpenFile(sFile1Path, "r");
	if (pFile != nullptr)
	{
		uReadBytes = ReadFile(pFile, sBuffer, uBUFFER_SIZE);
		...
		int iCloseFileResult = CloseFile(pFile);
	}
	else
	{
	...
	printf("\nEJERCICIO B\n\n");
	void* pFile2 = OpenFile(sFile2Path, "r");
	if (pFile2 != nullptr)
	{

		uReadBytes = ReadFile(pFile2, sBuffer, uBUFFER_SIZE);
