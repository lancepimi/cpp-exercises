...
TListNode::TListNode( const char *_sValue) 
	: m_pNext(nullptr)
{
	m_sValue = new char[strlen(_sValue) + 1];
	strcpy(const_cast<char*>(m_sValue), _sValue);
}
...
void TList::GetReverseList(TList& lstSrc, TList& nReturnList)
{
	TList nAuxList = lstSrc;

	unsigned int uSize = lstSrc.Size();
	for (unsigned int uIndex = 0; uIndex < uSize; uIndex++)
	{
		const char* sChain = nAuxList.Pop();
		unsigned int uLength = strlen(sChain);

		char* sText = new char[uLength + 1];
		memcpy(sText, sChain, uLength + 1);

		char* pStart = sText;
		char* pEnd = &sText[uLength - 1]; 
